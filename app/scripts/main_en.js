...
var vm = new Vue({
  ...
  methods: {
    ...
    importFileChanged: function(e) {
      ...
      if (extension !== '.json') {
        alert('Illegal extension! Please use *.json files from our website! ');
        return;
      }
...
function onThemeImported(result) {
  try {
    ...
    if (obj.themeName === undefined && obj.version === undefined) {
      alert('Please use the exported JSON configuration files from our site instead of downloaded theme file. ');
      return;
    }
    ...
    if (obj.version < VERSION) {
      ...
      if (unfound.length > 0) {
        alert('The imported theme have lower version, ' + unfound.length + ' attributes have not been set. The default values are now used. ');
      } else {
    ...
    alert('Illegal JSON format! Please use the exported JSON configuration files from our site. ');
    console.error(e);
  }
}
