...
function getIndicator() {
  const res = [];
  for (let i = 0; i < axisCat.length; ++i) {
    res.push({
      name: axisCat[i],
      max: 1000
    });
...
function getSeriesRandomValue(groupCnt: number, typeName: string) {
  const data = [];
  const dlen = typeName === 'scatter' ? 32 : axisCat.length;
  for (let i = 0; i < groupCnt; ++i) {
    let group = [];
    for (let j = 0; j < dlen; ++j) {
      ...
      if (typeName === 'scatter') {
        ...
        v = Math.floor(
          ((Math.random() * 600 + 400) * (groupCnt - i)) / groupCnt
        );
      }
      group.push(v);
    }
...
function getSeriesRandomStack(groupCnt: number, typeName: string) {
...
export function getPreviewOptions(groupCnt: number) {
  const options = [
    {
      title: {
        text: '折线图',
      ...
      series: [
        {
          ...
          data: (function() {
            ...
            for (let i = 0; i < groupCnt; ++i) {
              ...
              let cSum = 0;
              for (
                let c = 0, clen = Math.ceil(Math.random() * 5);
                c < clen;
                ++c
              ) {
                const grand = [];
                let gSum = 0;
                for (
                  let g = 0, glen = Math.ceil(Math.random() * 3);
                  g < glen;
                  ++g
                ) {
